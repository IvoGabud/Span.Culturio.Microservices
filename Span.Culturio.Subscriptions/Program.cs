...
Log.Logger = new LoggerConfiguration()
    ...
    .Enrich.WithProperty("Application", "Subscriptions Service")
    .WriteTo.Console()
    .WriteTo.File("logs/subscriptions-.txt", rollingInterval: RollingInterval.Day)
    .WriteTo.Seq("http://localhost:5341")
    .CreateLogger();

try
{
    Log.Information("Starting Subscriptions Service");
    ...
    builder.Services.AddSwaggerGen(c =>
    {
        ...
        {
            Title = "Culturio Subscriptions API",
            Version = "v1",
            Description = "Subscriptions management service"
        });
...
{
    Log.Fatal(ex, "Subscriptions Service terminated unexpectedly");
}
finally
{
    Log.CloseAndFlush();
